...
- **Cloudflare Worker (`anavvanzin`)**: `wrangler.jsonc` sets `main` to `worker.js` (KV-backed `GET`/`PUT /_state` for mesa persistence via binding `MESAS`) and `assets.directory` to `.` so Workers Builds can run plain `npx wrangler deploy` on a clean checkout. `assets.run_worker_first: ["/_state"]` + `assets.binding: "ASSETS"` keep everything else asset-first. Publication boundary is [`.assetsignore`](.assetsignore) (excludes `.git/`, tooling, agent notes, `worker.js`, and media over the **25 MiB per-asset** limit). New oversize media must be added to both `.assetsignore` and the strip list in `scripts/stage-worker-assets.sh` / `deploy-pages.yml`.
- Vercel Git deployments are disabled by `vercel.json` (`git.deploymentEnabled: false`); do not wait on or use Vercel previews for publication checks.
...
- `window-contents.js` — most window body components (`WSobre`, `WTese`, `WMae`, …), composed from the design-system bundle. Some components live in their own root file loaded separately by `mesa/index.html`, notably `ampulheta-native.js`.
